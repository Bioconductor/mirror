...
# Last Modified: 2015 Mar 31 12:16:09 PM
# Title:update-git.py
...
def reformat_branch_name(name):
  name_search = re.search("RELEASE_(\d+)_(\d+)")
  if name_search:
...
def track_branch(project, svn_branch, git_branch):
  subprocess.check_call(['git', 'config', '--add', 'svn-remote.{}.url'.format(git_branch),
                         '/'.join(['/branches/', svn_branch, args.prefix, project])])
...
def main():
  ...
  revision_info = subprocess.check_output(["svn", "log", "--verbose", "-r",
  ...
  if package_type == "trunk":
    ...
    raise Exception("Unknown package type: {}".format(package_type))
